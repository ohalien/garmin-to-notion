...
def activity_needs_update(existing_activity, new_activity):
    ...
    activity_type, activity_subtype = format_activity_type(new_activity.get('activityType', {}).get('typeKey', 'Unknown'), activity_name)
    has_sub = 'Subactivity Type' in props and props['Subactivity Type'].get('select') is not None

    return (
        props['Distance (km)']['number'] != round(new_activity.get('distance', 0) / 1000, 2) or
        props['Duration (min)']['number'] != round(new_activity.get('duration', 0) / 60, 2) or
        props['Calories']['number'] != round(new_activity.get('calories', 0)) or
        props['Avg Pace']['rich_text'][0]['text']['content'] != format_pace(new_activity.get('averageSpeed', 0)) or
        props['Avg Power']['number'] != round(new_activity.get('avgPower', 0), 1) or
        props['Max Power']['number'] != round(new_activity.get('maxPower', 0), 1) or
        props['Training Effect']['select']['name'] != format_training_effect(new_activity.get('trainingEffectLabel', 'Unknown')) or
        props['Aerobic']['number'] != round(new_activity.get('aerobicTrainingEffect', 0), 1) or
        props['Aerobic Effect']['select']['name'] != format_training_message(new_activity.get('aerobicTrainingEffectMessage', 'Unknown')) or
        props['Anaerobic']['number'] != round(new_activity.get('anaerobicTrainingEffect', 0), 1) or
        props['Anaerobic Effect']['select']['name'] != format_training_message(new_activity.get('anaerobicTrainingEffectMessage', 'Unknown')) or
        props['PR']['checkbox'] != new_activity.get('pr', False) or
        props['Fav']['checkbox'] != new_activity.get('favorite', False) or
        props['Activity Type']['select']['name'] != activity_type or
        (has_sub and props['Subactivity Type']['select']['name'] != activity_subtype) or
        (not has_sub)
    )
...
def update_activity(client, existing_activity, activity):
    start_time = datetime.fromisoformat(activity.get('startTimeGMT').replace("Z", "+00:00"))
    end_time = start_time + timedelta(seconds=activity.get('duration', 0))
...
def main():
    ...
    for activity in activities:
        date = activity.get('startTimeGMT')
        name = format_entertainment(activity.get('activityName', 'Unnamed Activity'))
        act_type, sub_type = format_activity_type(activity.get('activityType', {}).get('typeKey', 'Unknown'), name)

        existing = activity_exists(client, db_id, date, act_type, name)
        if existing:
            if activity_needs_update(existing, activity):
                update_activity(client, existing, activity)
        else:
            create_activity(client, db_id, activity)
